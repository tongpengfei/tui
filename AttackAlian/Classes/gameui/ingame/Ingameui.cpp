...
void Ingameui::onLoadScene(){
	...
	m_arrWeapon = DataProxy::sharedManager()->getArrWeapon();//获取装备
	m_arrWeapon->retain();
	m_arrMedicine = DataProxy::sharedManager()->getArrMedicine();//获取药水
	m_arrMedicine->retain();
	m_arrProp = DataProxy::sharedManager()->getArrProp();//获取装备
	m_arrProp->retain();
	m_curLevel = DataProxy::sharedManager()->getLevel();//获取关卡
